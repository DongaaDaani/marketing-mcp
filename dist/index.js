...
function getCredentials(req) {
    return {
        pageToken: req.headers["x-fb-page-access-token"] || DEFAULT_PAGE_TOKEN,
        pageId: req.headers["x-fb-page-id"] || DEFAULT_PAGE_ID,
        appId: req.headers["x-fb-app-id"] || DEFAULT_APP_ID,
        appSecret: req.headers["x-fb-app-secret"] || DEFAULT_APP_SECRET,
        apiVersion: req.headers["x-fb-api-version"] || DEFAULT_API_VERSION,
    };
}
...
function assertCredentials(creds) {
    const missing = [];
    if (!creds.pageToken)
        missing.push("FB_PAGE_ACCESS_TOKEN (x-fb-page-access-token header)");
    if (!creds.pageId)
        missing.push("FB_PAGE_ID (x-fb-page-id header)");
    if (missing.length)
...
function createMcpServer(creds) {
    ...
    server.tool("list_posts", "Visszaadja az oldal legutóbbi bejegyzéseit.", {
        ...
        try {
            ...
            const scheduled = include_scheduled && results[1] ? results[1].data.data ?? [] : [];
            const fmt = (p, type) => ({
                id: p.id, type,
    ...
    server.tool("create_post", "Uj bejegyzest tesz kozzé az oldalon képpel vagy anelkul, vagy utemezi. A kep megadhato base64 kodolt stringkent (image_base64), nyilvanos URL-kent (image_url), vagy link-kent.", {
        ...
        try {
            if (image_base64) {
                ...
                const res = await axios.post(`https://graph.facebook.com/${creds.apiVersion}/${creds.pageId}/photos`, formData);
    ...
    server.tool("get_post_insights", "Visszaadja egy bejegyzes statisztikait.", {
        ...
        try {
            ...
            for (const item of insightsRes.data.data ?? [])
                insightsMap[item.name] = item.values?.[0]?.value ?? item.values;
    ...
    server.tool("get_page_insights", "Visszaadja az oldal osszesitett statisztikait.", {
        ...
        try {
            ...
            const insightsMap = {};
            for (const item of insightsRes.data.data ?? [])
                insightsMap[item.name] = item.values ?? item.value;
            return { content: [{ type: "text", text: JSON.stringify({
...
function requireApiKey(req, res, next) {
    ...
    if (key !== SERVER_API_KEY) {
        res.status(401).json({ error: "Ervenytelen API kulcs." });
        return;
    }
    next();
}
...
app.post("/mcp", requireApiKey, async (req, res) => {
    ...
    try {
        ...
        if (!res.headersSent)
            res.status(500).json({ error: "Belso szerverhiba", details: String(err) });
    }
});
app.get("/mcp", requireApiKey, async (_req, res) => {
    res.status(405).json({ error: "A szerver stateless modban fut, GET nem tamogatott." });
});
app.delete("/mcp", requireApiKey, async (_req, res) => {
